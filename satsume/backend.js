// ==========================================
// Satsume Hub - 后端交互与 Discord 鉴权核心逻辑
// 文件位置: satsume/backend.js
// ==========================================
...
function renderDiscordUI() {
  ...
  if(isDiscordLoggedIn) {
    ...
    if(toggleSub) toggleSub.classList.remove('is-hidden');
    
    // ⚠️ 核心判定：以 satsume 是否被订阅为准
    if(subCheckbox) subCheckbox.checked = isProjectTracked('satsume');
...
function toggleSubscription() {
  const isChecked = document.getElementById('toggleReminder').checked;
  let subs = JSON.parse(localStorage.getItem('hub_subs')) || [];
  const projectId = 'satsume'; 
...
async function syncToBackend() {
  ...
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      let key = localStorage.key(i);
      if (key.startsWith('hub_done_') || key.startsWith('task_') || key.startsWith('sat_task_') || key.includes('_task_')) {
    ...
    await fetch(BACKEND_API_URL, {
      ...
      body: JSON.stringify({
        discord_id: userData.id, username: userData.username, reminder_on: isSubOn, tasks_done_today: allDone,
        xp: treeState.xp, level: treeState.level, streak: treeState.streak, last_active: treeState.lastActive,
        subs: subs, done_tasks: done_tasks
      })
    });
