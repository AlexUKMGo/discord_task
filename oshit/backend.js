// ==========================================
// OSHIT Hub - 后端交互与 Discord 鉴权核心逻辑
// 文件位置: oshit/backend.js
// ==========================================
...
function renderDiscordUI() {
  ...
  if(isDiscordLoggedIn) {
    ...
    // 智能提醒开关跟随当前项目的订阅状态
    if(subCheckbox) subCheckbox.checked = isProjectTracked('oshit');
...
function updateSubscriptionText() {
  ...
  if(!window.translations || !window.translations[currentLang] || !textEl) return;
  const data = window.translations[currentLang];
  textEl.textContent = isChecked ? data.sub_status_on : data.sub_status_off;
  textEl.className = isChecked ? "ml-3 text-xs font-bold text-emerald-400" : "ml-3 text-xs font-bold text-slate-400";
}

function toggleSubscription() {
  const isChecked = document.getElementById('toggleReminder').checked;
  let subs = JSON.parse(localStorage.getItem('hub_subs')) || [];
  const projectId = 'oshit';
...
async function pullFromBackend(discordId) {
  try {
    ...
    // 兼容调用：如果在子项目页面，则刷新 UI
    if (typeof updateProgressUI === 'function') updateProgressUI();
    if (typeof updateProjectTrackButton === 'function') updateProjectTrackButton();
    if (typeof renderDailies === 'function') renderDailies(); // 兼容主页
    if (typeof updateBonsaiUI === 'function') updateBonsaiUI(); // 兼容主页
    renderDiscordUI(); 
  }
}
